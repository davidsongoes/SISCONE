...
//MÉTODOS DE BUSCA
    ...
    static public function buscarMilitares()
    {
        ...
        foreach ($resultado as $result) {
            ...
            $efetivo->nome_completo = utf8_decode($result['nome_completo']);
            $efetivo->nome_guerra = utf8_decode($result['nome_guerra']);
...
$efetivo->rtcaer = $result['rtcaer'];
    ...
    static public function buscarPorId($id)
    {
        ...
        if ($result) {
            ...
            $efetivo->nome_completo = utf8_decode($result['nome_completo']);
            $efetivo->nome_guerra = utf8_decode($result['nome_guerra']);
            $efetivo->posto_graduacao = $result['id_post_grad'];
    ...
    public static function verificaFoto($id)
    {
        if (isset($id)) {
            $nome_arquivo = $id . '.jpg';
            if (file_exists("/var/www/padraointraer/fotos/$nome_arquivo")) {
    ...
    public static function validaFile($file)
    {
        ...
        if (empty($file["avatar-1"]['name'])) {
            $file = ["avatar-1" => [
                "name" => "sem_foto.jpg",
                "type" => "image/jpeg",
                "tmp_name" => "/var/www/padraointraer/fotos/sem_foto",
                "error" => 0,
                "size" => 20008
            ]
            ];
...
// Pasta onde o arquivo vai ser salvo
        $_UP['pasta'] = '/var/www/padraointraer/fotos/';
